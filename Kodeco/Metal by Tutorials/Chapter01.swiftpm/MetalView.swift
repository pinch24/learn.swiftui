...
struct MetalView: UIViewRepresentable {
	func makeUIView(context: Context) -> MTKView {
		...
		guard let mesh = try? MTKMesh(mesh: mdlMesh, device: device) else { fatalError() }
		...
		guard let library = try? device.makeLibrary(source: shader, options: nil) else { fatalError() }
