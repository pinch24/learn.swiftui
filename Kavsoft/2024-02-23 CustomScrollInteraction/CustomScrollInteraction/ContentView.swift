...
struct ContactSection: Identifiable {
	...
	static func generateData() -> [ContactSection] {
		Dictionary(grouping: dummyContacts) { String($0.name.first ?? "A") }
	...
	func CustomScrollIndicator() -> some View {
		GeometryReader { geometry in
			...
			.overlay(alignment: .topLeading) {
				ZStack {
					if showPopup {
						UnevenRoundedRectangle(topLeadingRadius: 50, bottomLeadingRadius: 50, bottomTrailingRadius: 5, topTrailingRadius: 50, style: .continuous)
							.fill(.blue.gradient)
		...
		.coordinateSpace(name: "INDICATORVIEW")
	}
}
